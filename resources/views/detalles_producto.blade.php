            ...
            <div class="w-full md:w-1/2 mt-4 md:mt-0">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    @if ($productDetails->image2)
                        <div class="bg-white rounded-lg shadow-md overflow-hidden">
                            <img src="{{ asset('images/' . $productDetails->image2) }}" alt="{{ $productDetails->name }}" class="w-full h-auto">
                        </div>
                    @endif
                    @if ($productDetails->image3)
                        <div class="bg-white rounded-lg shadow-md overflow-hidden">
                            <img src="{{ asset('images/' . $productDetails->image3) }}" alt="{{ $productDetails->name }}" class="w-full h-auto">
                        </div>
                    @endif
                </div>
            </div>
